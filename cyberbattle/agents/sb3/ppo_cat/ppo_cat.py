...
class CATPPO(OnPolicyAlgorithm):
    """
    Proximal Policy Optimization algorithm (PPO) (clip version) with Conditional Action Trees.

    Based on the original Stable Baselines 3 implementation.

    Introduction to PPO: https://spinningup.openai.com/en/latest/algorithms/ppo.html
    Background on Conditional Action Trees: https://arxiv.org/abs/2104.07294
    ...
    def _setup_model(self) -> None:
        ...
        if not isinstance(self.policy, VATActorCriticPolicy):
            raise ValueError("Policy must subclass `VATActorCriticPolicy`")
    ...
    def _setup_learn(
        ...
        :param use_vat: Whether to use valid action trees during training
        :return:
        """
        ...
        callback = self._init_callback(callback, eval_env, eval_freq, n_eval_episodes, log_path, use_vat)
    ...
    def collect_rollouts(
        ...
        :param use_vat: Whether or not to use valid action trees during training
        :return: True if function returned with at least `n_rollout_steps`
            collected, False if callback terminated rollout prematurely.
        """

        assert isinstance(
            rollout_buffer, VATRolloutBuffer
        ), "RolloutBuffer doesn't support valid action trees"
        assert self._last_obs is not None, "No previous observation was provided"
        # Switch to eval mode (this affects batch norm / dropout)
        self.policy.set_training_mode(False)
        n_steps = 0
        valid_action_trees = None
        rollout_buffer.reset()

        if use_vat and not is_vat_supported(env):
            raise ValueError("Environment does not support valid action trees. Consider using VATWrapper wrapper")
        ...
        while n_steps < n_rollout_steps:
            with th.no_grad():
                ...
                # This is the only change related to valid action trees
                if use_vat:
                    valid_action_trees = get_vat(env)

                actions, values, log_probs = self.policy(obs_tensor, valid_action_trees=valid_action_trees)
    ...
    def predict(
        ...
        valid_action_trees: Optional[np.ndarray] = None,
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                values, log_prob, entropy = self.policy.evaluate_actions(
                    ...
                    actions,
                    valid_action_trees=rollout_data.valid_action_trees,
                )
